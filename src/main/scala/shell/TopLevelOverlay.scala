...
case class TopLevelDesignInput(node: BundleBridgeSource[NexysVideoTopLevelIO])(implicit val p: Parameters)
...
class NexysVideoTopLevelIO extends Bundle {
  // Ethernet
  ...
  extends IOPlacedOverlay[ShellTopLevelPortIO, TopLevelDesignInput, TopLevelShellInput, TopLevelOverlayOutput]
{
  ...
  val tlSink: BundleBridgeSink[NexysVideoTopLevelIO] = sinkScope { di.node.makeSink }
